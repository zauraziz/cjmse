...
const ARTICLE_SELECT = `
  select a.slug, a.title, a.title_en, a.title_ru, a.type, a.pages, a.doi, a.views, a.citations,
         a.published_at, a.data_url, a.pdf_url,
         s.name_az as subject_az, s.name_en as subject_en, s.name_ru as subject_ru, s.slug as subject_slug,
         i.title as issue_title, i.volume as issue_volume, i.number as issue_number, i.year as issue_year,
...
export async function getSubjectsWithCounts() {
  return await q(`
    select s.slug, s.name_az, s.name_en, s.name_ru, count(a.id)::int as count
    from subjects s
...
export async function getArticleBySlug(slug) {
  const r = await q(
    `select a.*, s.name_az as subject_az, s.name_en as subject_en, s.name_ru as subject_ru, s.slug as subject_slug,
  ...
  const authors = await q(
    `select au.id as author_id, au.full_name, au.orcid, au.affiliation, au.research_group, aa.author_position, aa.is_corresponding
     from article_authors aa join authors au on au.id = aa.author_id
...
export async function getArticleEdit(id) {
  ...
  const authorLinks = await q(
    `select aa.author_id, aa.author_position, aa.is_corresponding,
            au.full_name, au.orcid, au.affiliation, au.research_group
     from article_authors aa join authors au on au.id = aa.author_id
     where aa.article_id = $1 order by aa.author_position`, [id]);
  const kw = await q(
...
export async function getAuthorPublic(id) {
  const r = await q(`select id, full_name, orcid, affiliation, research_group, email from authors where id = $1`, [id]);
  if (!r[0]) return null;
  const articles = await q(`
    select a.slug, a.title, a.title_en, a.title_ru, a.type, a.pages, a.published_at,
           i.title as issue_title, i.volume, i.number, i.year, aa.is_corresponding
    from article_authors aa
    join articles a on a.id = aa.article_id
    left join issues i on i.id = a.issue_id
    where aa.author_id = $1
    order by a.published_at desc nulls last`, [id]);
  return { ...r[0], articles };
}

export async function getAffiliations() {
  const r = await q(`select distinct affiliation from authors
                     where affiliation is not null and trim(affiliation) <> '' order by affiliation`);
  return r.map((x) => x.affiliation);
}

export async function getSubmissionByToken(token) {
  const r = await q(`select s.*, sub.name_az as subject_az, sub.name_en as subject_en, sub.name_ru as subject_ru
                     from submissions s left join subjects sub on sub.id = s.subject_id where s.token = $1`, [token]);
  return r[0] || null;
}
export async function getSubmissionsAdmin() {
  return await q(`select s.id, s.token, s.title, s.author_name, s.email, s.type, s.status, s.created_at, s.updated_at
                  from submissions s order by s.created_at desc`);
}
export async function getSubmissionById(id) {
  const r = await q(`select * from submissions where id = $1`, [id]);
  return r[0] || null;
}

export async function getReviewers() {
  return await q(`select id, full_name, email, affiliation from reviewers order by full_name`);
}
export async function getReviewerById(id) {
  const r = await q(`select * from reviewers where id = $1`, [id]);
  return r[0] || null;
}
export async function getReviewAssignmentsForSubmission(submissionId) {
  return await q(`
    select ra.*, r.full_name as reviewer_full, r.email as reviewer_mail
    from review_assignments ra
    left join reviewers r on r.id = ra.reviewer_id
    where ra.submission_id = $1
    order by ra.round, ra.created_at`, [submissionId]);
}
export async function getReviewAssignmentByToken(token) {
  const r = await q(`
    select ra.*, s.title as sub_title, s.abstract as sub_abstract, s.keywords as sub_keywords,
           s.type as sub_type, s.manuscript_url, s.manuscript_file_url, s.figures_urls, s.round as sub_round
    from review_assignments ra
    join submissions s on s.id = ra.submission_id
    where ra.token = $1`, [token]);
  return r[0] || null;
}

export async function getAdminAttention() {
  const newSubs = await q(`select id, title, author_name, created_at from submissions where status='submitted' order by created_at desc`);
  const reviewsReady = await q(`
    select distinct s.id, s.title, s.author_name
    from submissions s
    join review_assignments ra on ra.submission_id = s.id and ra.status='submitted' and ra.round = coalesce(s.round,1)
    where s.status='under_review' order by s.title`);
  const authorUpdates = await q(`select id, title, author_name, updated_at from submissions where flag_for_editor = true order by updated_at desc`);
  return { newSubs, reviewsReady, authorUpdates, total: newSubs.length + reviewsReady.length + authorUpdates.length };
}
export async function getAttentionCount() {
  const r = await q(`select
      ...
      + (select count(*) from submissions where flag_for_editor = true) as c`);
